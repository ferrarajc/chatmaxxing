...
function identifyTopFix(stats, codes, evaluations, runResults, exclude = null) {
  // Priority: highest weight first, then by severity, then alphabetical
...
function generateNextFix(iteration, score, thresholds, topFix, secondaryFix, criticalFailures) {
...
function generateReport(iteration, runResults, evaluations, stats, codes, score, thresholds, criticalFailures) {
  ...
  const status    = score >= thresholds.minOverallScore && !hasCritFails ? '✓ THRESHOLD MET' : '✗ BELOW THRESHOLD';
  ...
  md += `| Code | Name | Severity | Weight | Pass | Marginal | Fail | N/A |\n`;
  md += `|------|------|----------|--------|------|----------|------|-----|\n`;
  ...
  for (const [code, def] of Object.entries(codes)) {
    ...
    md += `| ${code} | ${def.name} | ${def.severity ?? ''} | ×${def.weight ?? 1} | ${s.pass} | ${s.marginal} | ${failMark} | ${s.na} |\n`;
  }
...
/**
 * Write report-NNN.md, latest.json, and NEXT_FIX.md to the results directory.
 *
 * @param {object}   opts
 ...
 * @param {object}   opts.thresholds     - { minOverallScore, zeroCriticalCodes, highSeverityPassRate? }
...
export function writeReports({ iteration, runResults, evaluations, heuristics, thresholds, resultsDir }) {
  ...
  // 1. Human report
  const reportMd   = generateReport(iteration, runResults, evaluations, stats, codes, score, thresholds, critFails);
  ...
  // 3. NEXT_FIX.md
  const nextFixMd = generateNextFix(iteration, score, thresholds, topFix, secFix, critFails);
