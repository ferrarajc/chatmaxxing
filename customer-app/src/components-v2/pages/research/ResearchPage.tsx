        ...
        {filtered.map(fund => (
          <div key={fund.ticker} style={{ background: theme.color.surface, borderRadius: theme.radius.lg, padding: '24px', boxShadow: theme.shadow.sm, border: `1px solid ${theme.color.border}` }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
              <div>
                <div style={{ fontWeight: 600, fontSize: 17, marginBottom: 4, color: theme.color.text, fontFamily: theme.font.serif, letterSpacing: '-0.01em' }}>{fund.name}</div>
              ...
              <span style={{ fontWeight: 700, fontSize: 14, color: theme.color.accent, fontFamily: theme.font.mono, letterSpacing: '0.04em', background: theme.color.accentSoft, padding: '4px 8px', borderRadius: theme.radius.sm }}>{fund.ticker}</span>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: 8, marginBottom: 18 }}>
              {[
                ...
                <div key={r.label} style={{ textAlign: 'center', background: theme.color.surfaceMuted, borderRadius: theme.radius.md, padding: '10px 4px' }}>
                  <div style={{ fontSize: 15, fontWeight: 600, color: r.value > 0 ? theme.color.success : theme.color.danger, fontFamily: theme.font.serif, fontVariantNumeric: 'tabular-nums' }}>
                    {r.value > 0 ? '+' : ''}{r.value}%
                  </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 13, color: theme.color.textMuted, paddingTop: 14, borderTop: `1px solid ${theme.color.border}`, fontVariantNumeric: 'tabular-nums' }}>
              <span>Expense ratio: <strong style={{ color: theme.color.text, fontWeight: 600 }}>{fund.expenseRatio}%</strong></span>
