/**
 * Automated test for the initiate-rollover task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-initiate-rollover.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-015', name: 'Alex Johnson', phone: '4842384838',
...
You want to roll over your old 401(k) from Fidelity (about $42,000) into your Roth IRA at Bob's.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked source institution: say "Fidelity" or "my old 401k at Fidelity".
- When asked source account type: say "traditional 401k" or "401(k)".
- When asked estimated amount: say "about $42,000" or "roughly forty-two thousand".
- When asked target account: confirm your Roth IRA.
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to roll over my old 401k into my account here';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'rollover roll over 401k transfer in incoming',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.sourceInstitution || byKey.sourceInstitution.trim() === '') issues.push('sourceInstitution missing');
    if (!byKey.sourceAccountType || byKey.sourceAccountType.trim() === '') issues.push('sourceAccountType missing');
    if (!byKey.estimatedAmount || byKey.estimatedAmount.trim() === '') issues.push('estimatedAmount missing');
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to bring their retirement savings from a former employer into their account',
  'Client wants to transfer money from their previous job\'s plan to Bob\'s',
  'The client would like to consolidate a 401k from another company into their IRA here',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated initiate-rollover conversations...\n`);
