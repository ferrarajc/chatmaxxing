/**
 * Automated test for the cancel-reschedule-callback task (reschedule path).
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-cancel-reschedule-callback.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-018', name: 'Alex Johnson', phone: '4842384838',
...
You want to reschedule your callback from its current time to Friday at 2 PM.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked cancel or reschedule: say "reschedule" or "move it to a different time".
- When asked new time: say "Friday at 2 PM" or "this Friday afternoon".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I need to change the time of my scheduled callback';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'reschedule callback cancel callback change my callback',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.action || !/reschedule/i.test(byKey.action)) issues.push(`action wrong: "${byKey.action}"`);
    if (!byKey.newScheduledTime || byKey.newScheduledTime.trim() === '') issues.push('newScheduledTime missing');
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to move their scheduled phone call to a different time',
  'Client wants to push back their upcoming call with an agent',
  'The client needs to change when they will be called back',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated cancel-reschedule-callback conversations...\n`);
