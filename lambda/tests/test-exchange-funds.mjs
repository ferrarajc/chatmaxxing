/**
 * Automated test for the exchange-funds task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-exchange-funds.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-007', name: 'Alex Johnson', phone: '4842384838',
...
You want to exchange $5,000 from BF500 into BFBI (the bond income fund) within your Roth IRA.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked source fund: say "BF500" or "my 500 index fund".
- When asked destination fund: say "BFBI" or "the bond income fund".
- When asked the amount: say "$5,000".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to exchange some of my 500 index fund into bonds';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'exchange switch funds reallocate transfer between funds',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.fromFund || !/BF500/i.test(byKey.fromFund)) issues.push(`fromFund wrong: "${byKey.fromFund}"`);
    if (!byKey.toFund || !/BFBI/i.test(byKey.toFund)) issues.push(`toFund wrong: "${byKey.toFund}"`);
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to move money from one fund to another',
  'Client wants to shift some assets from stocks to bonds',
  'The client would like to swap their index fund holdings into a different fund',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated exchange-funds conversations...\n`);
