/**
 * Automated test for the update-auto-invest task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-update-auto-invest.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-010', name: 'Alex Johnson', phone: '4842384838',
...
You want to change your monthly $200 BF500 auto-invest to $350 per month, keep the same frequency and day.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked which schedule: say "the monthly $200 into BF500 in my Roth IRA".
- When asked new amount: say "$350".
- When asked frequency: say "keep it the same" or "still monthly".
- When asked day of month: say "keep the same day".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to increase my automatic investment amount';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'change auto invest update automatic modify recurring investment schedule',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.dayOfMonth || byKey.dayOfMonth.trim() === '') issues.push('dayOfMonth missing');
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to put more money into their automatic savings plan',
  'Client wants to adjust the amount on their scheduled investment',
  'The client would like to change how much goes in automatically each month',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated update-auto-invest conversations...\n`);
