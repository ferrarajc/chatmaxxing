/**
 * Automated test for the update-contact-info task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-update-contact-info.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-002', name: 'Alex Johnson', phone: '4842384838',
...
You want to update your email address to alex.johnson.new@gmail.com.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked what to update: say "my email address".
- When asked for the new email: say "alex.johnson.new@gmail.com".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I need to update my email address';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'update email address change contact info',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.infoType || !/email/i.test(byKey.infoType)) issues.push(`infoType wrong: "${byKey.infoType}"`);
    if (!byKey.newValue || !/alex\.johnson\.new@gmail\.com/i.test(byKey.newValue)) issues.push(`newValue wrong: "${byKey.newValue}"`);
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer needs to change their email on file to a new address',
  'Client wants to update their contact details',
  'The client would like to change their email to a different one',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated update-contact-info conversations...\n`);
