/**
 * Automated test for the open-account task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-open-account.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-004', name: 'Alex Johnson', phone: '4842384838',
...
You want to open a Traditional IRA, start with $5,000, funded via bank transfer.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked account type: say "Traditional IRA".
- When asked initial amount: say "$5,000" or "five thousand dollars".
- When asked funding source: say "bank transfer" or "ACH".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to open a new account';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'open account new account traditional IRA',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.accountType || !/traditional/i.test(byKey.accountType)) issues.push(`accountType wrong: "${byKey.accountType}"`);
    if (!byKey.initialAmount || byKey.initialAmount.trim() === '') issues.push('initialAmount missing');
...
const LLM_FALLBACK_INTENTS = [
  'The customer would like to start a new IRA account with us',
  'Client wants to create a retirement savings account',
  'The client is interested in opening an investment account at our firm',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated open-account conversations...\n`);
