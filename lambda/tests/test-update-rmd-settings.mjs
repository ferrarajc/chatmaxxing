/**
 * Automated test for the update-rmd-settings task.
 * Requires a Traditional IRA client (RMD only applies to Traditional IRA / SEP-IRA).
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-update-rmd-settings.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-014', name: 'Maria Chen', phone: '2155551234',
...
You want to update your RMD to: monthly payments, sent by direct deposit, with 15% federal tax withholding.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked delivery method: say "direct deposit" or "to my bank".
- When asked frequency: say "monthly".
- When asked tax withholding: say "15 percent" or "15%".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to update my required minimum distribution settings';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'rmd required minimum distribution update rmd settings',
      }),
    });
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to change how they receive their mandatory annual withdrawal',
  'Client needs to update the delivery for their required distributions',
  'The client wants to change when and how their minimum distributions are paid out',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated update-rmd-settings conversations...\n`);
