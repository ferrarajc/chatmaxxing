/**
 * Automated test for the roth-conversion task.
 * Requires a Traditional IRA client.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-roth-conversion.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-016', name: 'Maria Chen', phone: '2155551234',
...
You want to convert $25,000 from your Traditional IRA to a Roth IRA for tax year 2026.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked source account: say "my Traditional IRA" or "the traditional one".
- When asked amount: say "$25,000".
- When asked tax year: say "2026".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to convert some of my traditional IRA to a Roth';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'roth conversion convert to roth traditional to roth',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.fromAccountId || byKey.fromAccountId.trim() === '') issues.push('fromAccountId missing');
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to move pre-tax retirement savings into a tax-free account',
  'Client wants to do a backdoor Roth conversion',
  'The client would like to convert their traditional IRA funds over to a Roth',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated roth-conversion conversations...\n`);
