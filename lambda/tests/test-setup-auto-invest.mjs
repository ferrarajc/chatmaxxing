/**
 * Automated test for the setup-auto-invest task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-setup-auto-invest.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-009', name: 'Alex Johnson', phone: '4842384838',
...
You want to set up a monthly automatic investment of $200 into BF500 on the 15th of every month.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked which fund: say "BF500" or "the 500 index fund".
- When asked the amount: say "$200 per month".
- When asked frequency: say "monthly".
- When asked day of month: say "the 15th".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to set up automatic monthly investments into my Roth IRA';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'set up auto invest automatic investment recurring monthly',
      }),
    });
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants money to automatically go into their retirement account each month',
  'Client wants to schedule a recurring investment',
  'The client would like to start investing a fixed amount on a regular basis',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated setup-auto-invest conversations...\n`);
