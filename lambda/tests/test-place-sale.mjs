/**
 * Automated test for the place-sale task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-place-sale.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-006', name: 'Alex Johnson', phone: '4842384838',
...
You want to sell $8,000 of the BFGR (growth fund) from your Roth IRA for rebalancing purposes.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked which fund: say "BFGR" or "the growth fund".
- When asked the amount: say "$8,000".
- When asked reason: say "rebalancing" or "I want to rebalance my portfolio".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to sell some of my growth fund shares';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'sell redeem shares sell my fund',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.fund || !/BFGR/i.test(byKey.fund)) issues.push(`fund wrong: "${byKey.fund}"`);
    if (!byKey.amount || byKey.amount.trim() === '') issues.push('amount missing');
    if (!byKey.reason || byKey.reason.trim() === '') issues.push('reason missing');
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to get out of one of their funds',
  'Client wants to liquidate some shares in their IRA',
  'The client would like to cash out part of their investment',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated place-sale conversations...\n`);
