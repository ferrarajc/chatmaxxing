/**
 * Automated test for the update-beneficiaries task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-update-beneficiaries.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-003', name: 'Alex Johnson', phone: '4842384838',
...
You want to add your daughter Maria Johnson as a 100% primary beneficiary on your Roth IRA.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked what to do: say "add a beneficiary".
- When asked for name: say "Maria Johnson".
- When asked for relationship: say "my daughter".
- When asked for percentage: say "100 percent" or "all of it".
- When asked primary or contingent: say "primary".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to add a beneficiary to my account';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'add beneficiary update designation',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.action) issues.push('action missing');
    if (!byKey.beneficiaryName || !/maria johnson/i.test(byKey.beneficiaryName)) issues.push(`beneficiaryName wrong: "${byKey.beneficiaryName}"`);
    if (!byKey.relationship || byKey.relationship.trim() === '') issues.push('relationship missing');
    if (!byKey.percentage || byKey.percentage.trim() === '') issues.push('percentage missing');
    if (!byKey.beneficiaryType || byKey.beneficiaryType.trim() === '') issues.push('beneficiaryType missing');
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants to name someone as the person who will inherit the account',
  'Client needs to designate who gets the money when they pass away',
  'The client would like to set up a beneficiary for their investment account',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated update-beneficiaries conversations...\n`);
