/**
 * Automated test for the toggle-drip task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-toggle-drip.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-008', name: 'Alex Johnson', phone: '4842384838',
...
You want to turn on dividend reinvestment for the BF500 fund in your Roth IRA.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked which fund: say "BF500" or "my 500 index fund".
- When asked turn on or off: say "turn it on" or "I want to reinvest the dividends".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to turn on dividend reinvestment for one of my funds';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'dividend reinvestment drip reinvest',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.dripEnabled || byKey.dripEnabled.trim() === '') issues.push('dripEnabled missing');
    else if (!/on|reinvest/i.test(byKey.dripEnabled)) issues.push(`dripEnabled should be ON: "${byKey.dripEnabled}"`);
  }
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants dividends to be automatically put back into shares',
  'Client wants to stop receiving dividend cash payments and reinvest instead',
  'The client would like to change how dividends are handled for a fund',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated toggle-drip conversations...\n`);
