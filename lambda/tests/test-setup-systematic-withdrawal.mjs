/**
 * Automated test for the setup-systematic-withdrawal task.
 * Pass threshold: 9/10 runs
 * Usage: OPENAI_API_KEY=sk-... node lambda/tests/test-setup-systematic-withdrawal.mjs
 */
...
const CLIENT_PROFILE = {
  clientId: 'test-013', name: 'Maria Chen', phone: '2155551234',
...
You want to set up monthly withdrawals of $800 from your Traditional IRA, starting July 1st, paid by direct deposit.

Rules:
- Answer questions naturally. Give one piece of info per reply.
- When asked amount: say "$800 per month" or "eight hundred dollars".
- When asked frequency: say "monthly".
- When asked start date: say "July 1st" or "starting in July".
- When asked delivery: say "direct deposit" or "to my bank account".
...
async function runSingleTest(runIndex, intentOverride) {
  const transcript = [];
  let systemMessages = [];
  const initialMessage = 'I want to set up automatic monthly withdrawals from my IRA for retirement income';
  ...
  for (let turn = 0; turn < MAX_TURNS; turn++) {
    ...
    const res = await fetch(API_URL, {
      ...
      body: JSON.stringify({
        ...
        scope: 'get-intent', currentIntent: intentOverride ?? 'systematic withdrawal recurring withdrawal monthly withdrawal schedule',
      }),
    });
  ...
  if (!proposedAction) { issues.push('no proposedAction returned'); }
    ...
    if (!byKey.frequency || byKey.frequency.trim() === '') issues.push('frequency missing');
    if (!byKey.startDate || byKey.startDate.trim() === '') issues.push('startDate missing');
...
const LLM_FALLBACK_INTENTS = [
  'The customer wants a steady income stream from their retirement account each month',
  'Client wants regular automatic payments sent to their bank from their IRA',
  'The client would like recurring distributions set up from their investment account',
];

async function main() {
  const RUNS = 10; const PASS_THRESHOLD = 9;
  console.log(`\nRunning ${RUNS} simulated setup-systematic-withdrawal conversations...\n`);
