...
| `client-data` | Read/write client profile. Also `get-contributions` — the IRA contribution summary (aggregated across ALL of a client's IRAs, per IRS rules), computed by `shared/contributions.ts`; optional `asOf` shifts the evaluation date so the prior-tax-year path is testable outside Jan–Apr | No |
...
| `fund-data-refresh` | **Nightly real-market-data mirror.** Fetches each fund's Vanguard proxy (`FundDef.realSymbol`) from Yahoo — full daily history back to inception (chart API) + holdings/sectors/characteristics (crumb-authenticated quoteSummary, fetched once/run) — computes quote/trailing/annual returns/chart series/distributions/beta/stdDev/yield and writes `bobs-fund-market` (year-chunked history, hash-guarded write-if-changed; a failed fund writes NOTHING so last good data survives). Runs via EventBridge Scheduler cron 07:10 UTC (both stages); manual: GET `/refresh-fund-data?key=bobs-reset-2025[&ifStaleMinutes=N]` → 202 + async self-invoke (API GW 29s cap; a run takes ~1-2 min). | No |
| `fund-market` | Read API over `bobs-fund-market`: GET `/fund-market?ticker=X` (page-ready `FundMarketData`) / `?summary=1` (lineup slice) / `?status=1` (last refresh-run status, uncached — poll after triggering a refresh). Read-only by design (never holds write perms). | No |
...
0. **`forceTaskId` in the request wins outright** — the agent picked that task expert from the ✈ menu,
   so classification is skipped, the ADVICE/callback-intent redirects are bypassed, and any existing
   `[TASK:]` marker is ignored (this run starts the chosen expert fresh). Unknown ids fall through to 1.
   ⚠️ **`forceTaskId` is sent on the KICKOFF TURN ONLY** (`ChatColumn.tsx` clears it once dispatched);
   every later turn of the same task is carried by the `[TASK:]` marker alone. So a bypass keyed to
   `forceTaskId` protects one turn, not the task. The **advice redirect is therefore also gated on
   `activeTaskId`**, which is resolved from the marker *before* the guard runs — otherwise the guard
   fires on mid-task client messages and kills the expert (it did: "Which fund do I hold right now"
   during a buy-funds task returned the scripted advice decline). Once an expert is running, advice is
   handled inside the expert prompt by `FORBIDDEN_TOPICS`, which has the conversation's context.
...
The `[TASK: id]` transcript marker that drives Phase 2 is read **last-marker-wins**, so a chat can run
several tasks in sequence (finish beneficiaries, then start account access).
...
| Adding a new task | `lambda/shared/tasks.ts` (add task) + `handler.ts` (add prompt + switch case) + `execute-task/handler.ts` (add execution). The task appears in the agent's ✈ autopilot menu automatically (labeled with its `Task.name`); add a short verb+noun label to `TASK_MENU_LABELS` in `agent-app/src/data/automations.ts` to give it agent-facing wording. |
| Task execution logic | `lambda/execute-task/handler.ts` |
| Agent chat rendering | `agent-app/src/components/ChatColumn.tsx` |
| Autopilot ✈ menu (agent app) | `agent-app/src/components/AutopilotMenu.tsx` — lists **every** automation, not just the 5 scopes: 4 **Global** automations (🌐 globe icon; Get intent, Idle check, Researching, Full auto — fixed order, listed first) then all **Task** automations (📄 document icon; alphabetical). Header + filter box are pinned; everything below them shares ONE scroll area — an earlier version pinned the Global block and starved the task list of height in a real agent column. An auto-focused filter box at the top matches label + the task's own `keywords` + description (↑/↓/Enter/Esc; a query collapses both groups into one ranked list). Positioning flips above/below with a `maxHeight` cap (ported from `ChangeToMenu.tsx`) since the list is now ~24 rows. Catalog: `agent-app/src/data/automations.ts`, which **imports `lambda/shared/tasks.ts` directly** (the reverse of the `lambda/shared/fund-catalog.ts` bridge — tasks.ts is pure, import-free data) so the menu can never drift from the registry; `TASK_MENU_LABELS` there supplies agent-facing verb+noun labels and may map one task to several rows (`cancel-reschedule-callback` shows as both *Cancel callback* and *Reschedule callback* — same expert, it infers which from the transcript). Selecting a Task row sets `ContactSlot.pendingTaskId`, which `ChatColumn`'s activation effect sends as **`forceTaskId`** on `/autopilot-turn` so that expert starts immediately against the live transcript — the agent no longer has to route through Get intent and hope the classifier picks it. The ✈ header label now reads the running TASK (`activeAutomationLabel`), e.g. "Change beneficiaries", not the scope it rides on. |
| Autopilot controls (agent app) | `ChatColumn.tsx` — during autopilot the (unused) composer is replaced by a big **Pause Autopilot** button lifted above the green overlay (`zIndex:2`); paused → red **Exit** + green **Resume ▶** (resumes from the remaining time at pause). **There is no column click-to-exit** — autopilot is changed only by these buttons + the `AISupport` ✈ toggle, so clicking/resizing elsewhere never exits it. A live **send countdown** (top-right of the box, `m:ss`; in the final 10s the ⏳ hourglass blinks while the time goes solid bright-red; gray/frozen while paused) renders via `AutopilotCountdown.tsx` inside the "Autopilot sending…" box in `AISupport.tsx` (grid, now a **white card + shadow** floating above the green overlay) + `FocusingDesktop.tsx` (focusing). State on `ContactSlot`: `autopilotPaused` / `autopilotSendAt` / `autopilotPausedRemainingMs`, reset at every autopilot clear/activate site. The `autopilotDelay` waiter is a 250 ms **pause-aware poll loop** (freezes on pause, shifts the deadline on resume) — the two-phase reading+typing delay durations are unchanged. **Paused control bar (`AISupport.tsx`, grid):** while paused, the "Autopilot sending…" box grows a control bar — the timer shifts left, a **‹ › pager** takes the top-right, **Send now** sits bottom-left (fires the displayed reply immediately via a one-shot `autopilotSendNow` flag the send loop consumes on its next poll), and **🪄 Magic / Change to ▼** sit bottom-right (same `next-best-response` modes as the Suggested-reply box, operating on a **parallel autopilot deck**: `autopilotHistory`/`autopilotIndex` + store actions `initAutopilotDeck`/`paginateAutopilot`/`editCurrentAutopilotCandidate`/`addAutopilot{ChangeTo,Magic}`/`setAutopilotChangeOptions(Loading)`; `autopilotPending` mirrors the shown candidate; change-options pre-generate on pause). On Resume it collapses back and the countdown continues. **Engine resilience (shipped with the control bar, because pause-to-edit invites heavy editing):** a task-expert LLM/parse error no longer bails mid-task (backend Phase-2 `catch` in `autopilot-turn/handler.ts` now keeps autopilot on with a holding reply, mirroring the Phase-1 catch); `runAutopilotTurn` **retries once** before exiting (and exits with a resumable message); `initRunRef` is **reset on exit** and re-activation runs a **fresh turn when a `[TASK:]` marker exists**, so a stopped task resumes instead of dead-green no-op; a post-`await` scope re-check stops stale turns resurrecting state; and a per-contact **send-generation guard** (`autopilotGenRef`) makes a customer reply arriving mid-edit supersede the older send loop (no double-send, no clobbered edit). Grid-only (`AISupport`); FocusingDesktop keeps the read-only pending box (parity deferred). |
...
| Suggested reply — "Change to" / "Magic" / auto-refresh / telemetry (agent app) | Two dropdowns on the Suggested reply box (both portal + space-aware up/down): **Change to ▼** (`ChangeToMenu.tsx`) alters the reply's MEANING; **🪄 Magic ▼** (`MagicMenu.tsx`) keeps the meaning and restyles it (presets concise/detailed/casual/formal + a custom field). Change-to options are pre-generated AFTER a suggestion shows (zero added latency; cached per entry). Backend: `next-best-response` gained a `mode` — `change-options` (short, tightly on-topic direction labels; names the client), `change-reply` (author a new reply along a direction, tools like NBR), `magic-rewrite` (cheap restyle, substance unchanged); the default suggest path writes **as the agent, never the client's voice**, and its prompt now **bans empty placeholder replies** ("feel free to ask", "anything else I can help with?") — when the agent spoke last or the client was just fully answered, it must MOVE THE CONVERSATION FORWARD with substance (next relevant detail / related consideration / useful question / concrete next step) rather than stall (same bland-filler failure the `change-reply` fix addressed, generalized to the always-on path). **`change-reply` uses its own direction-first `CHANGE_REPLY_SYSTEM` prompt, NOT the base suggest prompt** — reusing the base prompt made gpt-4o-mini ignore the chosen direction (its "1-2 sentences max" + "if the agent spoke last, just add a brief follow-up" rules produced bland "feel free to ask" filler); the dedicated prompt makes the picked direction the whole task, neutralizes those two rules, and receives the rejected draft (`currentSuggestion`) so it doesn't repeat it. History entries are `Suggestion` objects (`{id,text,originalText,source,changeDirection?,magicStyle?,changeOptions...}`); store actions `addChangeToReply`/`addMagicReply`/`setChangeOptions(Loading)`. **Auto-refresh:** a manual send (suggested Send OR composer Send, NOT resource links) fires `runNbrRefresh` immediately since the shown suggestion is stale. **Telemetry:** every agent send is recorded to the **`bobs-reply-events`** table via the **`log-reply`** Lambda + `agent-app/src/api/replyLog.ts` (how it was produced: suggestion as-is / edited / change-to / magic / composer freehand / autopilot). |
...
| Research fund lineup (36 funds) | **Canonical source: `customer-app/src/data/funds.ts`** (`FUNDS: FundDef[]`, pure data — keep it React-free). It is the single edit point, the **seed** for the `bobs-funds` DynamoDB table, and the frontend **offline fallback**. At runtime the table is the source of truth: served by `GET /funds` (`lambda/get-funds`, module-cached) and consumed via the **`useFunds()` hook** (`customer-app/src/hooks/useFunds.ts`, localStorage TTL + bundled fallback). Seed/refresh per-env with `GET /reset-funds?key=bobs-reset-2025` (`lambda/reset-funds`). The backend reaches `funds.ts` through the **only** cross-package bridge, `lambda/shared/fund-catalog.ts` (re-exports `FUNDS` + `FUND_PICKLIST`). AI access: the **`get_funds`** tool in `lambda/shared/client-tools.ts` (reads `bobs-funds`, available to bot + NBR + task experts), and task-expert prompts inject `FUND_PICKLIST`. Live prices/returns still come from `lambda/market-data/handler.ts` → `FUND_MAP` (ticker→Vanguard realSymbol, Yahoo quotes), which is now **derived from the catalog** (`FUNDS.map(...)` via `fund-catalog.ts`) — no longer hand-maintained, so it can't drift from the lineup. **Order execution** uses `FUND_PRICES`, also in `fund-catalog.ts` and also derived (`FundDef.seedNav` — the demo NAV that prices both seeded holdings and new orders). It used to be a hand-written object in `client-defaults.ts` holding only the ORIGINAL SIX tickers while the experts offered all 36: `matchFund()` returned null for the other 30 and `execute-task` took a fallback that answered "order placed" while writing **nothing** — no holding, no balance change, no transaction row, so an IRA contribution in one of those funds never reached the contributions card. Adding a fund to `funds.ts` (with a `seedNav`) is now the only step needed to make it tradeable. The registry had the SAME stale-six gap in a different field: six `Task.fields` (`place-purchase.fund`, `place-sale.fund`, `exchange-funds.fromFund`/`.toFund`, `toggle-drip.fund`, `setup-auto-invest.fund`) hard-coded `['BF500','BFGR','BFBI','BFIN','BFESG','BFST']` as their enum `options` while the prompts offered all 36 — the registry and the conversation disagreed about what a client could ask for. They now use **`FUND_TICKERS`**, also derived in `fund-catalog.ts`. That makes `tasks.ts` a cross-package importer, so `scripts/task-sim/src/facts.mjs` bundles it with esbuild instead of importing it directly. Pages that show fund data (`/help/fees`, `/help/fund-performance`, `/help/prospectus`, `/resources/tax-efficient-investing`, `OpenAccountPage`, plus Research/`FundProfilePage`) all read via `useFunds()`. |
| Real fund market data (nightly cache → fund pages) | **The fund profile pages show REAL market data** mirrored from each fund's Vanguard proxy. Pipeline: `lambda/fund-data-refresh` (nightly EventBridge 07:10 UTC → Yahoo → `bobs-fund-market`) → `.github/workflows/refresh-fund-data.yml` (07:30 UTC cron: `scripts/fund-data/export.mjs` pulls `GET /fund-market` per ticker, validates ALL before writing ANY, publishes to gh-pages **`/fund-data/*.json`** with `keep_files` + the shared `concurrency: gh-pages` group). Frontend: `hooks/useFundMarket.ts` reads the same-origin static file → falls back to `GET /fund-market` → falls back to the pre-existing behavior (useMarketData + static funds.ts), so a missing cache can never break a page. FundProfilePage merge rule: **intraday (price/dayChange) prefers live**, historical/derived (returns/holdings/allocations/distributions/beta/chart) **prefers the nightly payload**. New chart: `research/PriceHistoryChart.tsx` (recharts, range pills, renders only when data exists). Local dev: `API_URL=<dev api> npm run fund-data` fills `customer-app/public/fund-data/` (gitignored — committing it would ship a stale copy in dist/ and stomp the published files). **ADDING A FUND:** one entry in `funds.ts` (with `realSymbol`) → deploy → `GET /reset-funds?key=…` → `GET /refresh-fund-data?key=…` — the page works immediately via the API fallback; the static file appears after the next nightly workflow. Nothing else to touch. |
| IRA contributions summary (customer account pages + AI) | **One server-side computation, many consumers.** Annual figures + deadlines + account classification live in `lambda/shared/contribution-limits.ts` (PURE — the single edit point for IRS numbers, imported by both packages); the aggregation lives in `lambda/shared/contributions.ts` (`computeContributionSummary(clientId, asOf?)`, one Query per tax year over `bobs-transactions` using the date-prefixed `txnSort` — no GSI, no schema change). Exposed three ways, all reading that one function: the `get-contributions` action on `client-data` (UI), the **`get_contribution_room`** tool in `client-tools.ts` (reaches every conversational surface — 19 task experts, full-auto, customer-bot/voice, NBR, prep-callback — because all three tool-using Lambdas pass the catalog wholesale), and `CONTRIBUTION_DATA_RULE` appended at autopilot-turn's 3 LLM call sites (+ `NBR_CONTRIBUTION_RULE` in next-best-response) so the model calls the tool instead of asking the client for their age. **Counting rule:** `type:'contribution'` on a Roth/Traditional account; SEP is a SEPARATE bucket with its own limit and NO computed remaining (25% of compensation, which we don't hold); `dividend`/`purchase`/`exchange`/`deposit` excluded; `Scheduled` reported but not counted. UI: `components-v2/pages/account/ContributionsCard.tsx` + `hooks/useContributions.ts`, rendered on `AccountDetailPage` for IRA/SEP accounts. **Contribute →** goes straight into the order form at **`/contribute?account=<id>`** — the same `BuyPage`, entered without a route ticker, which is the mode switch into fund-picker mode. "Contribute" promises a transaction, so it must not open a browse list (it did in #125; corrected in #126). The page leads with **Invest Into → Fund → Funding Source → Amount** — account first because it decides which held funds are offered — and the Fund section lists the funds already held in the selected account with their positions, plus **A different fund →** out to the lineup. That detour carries the whole in-progress order (`account`/`amount`/`type`/`freq`) via `customer-app/src/utils/buyIntent.ts` (`readBuyIntent`/`buyIntentQuery`, the one spelling shared by `BuyPage`, `ResearchPage` and `FundProfilePage`), so the client returns to their work rather than an empty form. A fund chosen elsewhere renders as a selected row marked "Not currently held in this account" so an account switch can't silently drop it; an account with no holdings gets an in-page empty state, never a redirect. Each bucket's Contribute resolves its OWN target account via `classifyAccount` over the persona's accounts — on a SEP page the personal Traditional/Roth block targets the Roth, not the SEP (that was a real bug in #125: it would have deposited money that doesn't count toward the limit shown above it). IRA buys now write `type:'contribution'` with a POSITIVE amount in both `clientStore.buyFund` and `execute-task` place-purchase, so a contribution feeds back into the figure. Limits prose on `IraContributionLimitsPage`/`IraLimitsHelpPage`/`OpenAccountPage`/`AutoInvestPage` and the `kb.ts` answers all interpolate from the shared table, so nothing can quote a stale year. Tests: `lambda/tests/test-contribution-limits.mjs` (offline) + `test-contributions.mjs` (API). |
| Financial-advice guard | `lambda/shared/advice-guard.ts` — `ADVICE_RE` + `isAdviceRequest()`. Previously an identical regex copy-pasted into `autopilot-turn` and `next-best-response`; both now import it. Carries a **contribution-room carve-out**: `ADVICE_RE` matches `invest` inside `investment`, so "What can I still invest in my IRA this year?" was force-routed to a callback before any LLM call. Contribution limits are tax-rule facts, not recommendations, so that phrasing is exempted — unless a strong advice marker is also present ("what's the BEST FUND for my contribution?"), which keeps fund-selection questions on the callback path. Also carries a **current-holdings carve-out**: what a client already owns is a fact (`get_holdings` exists to answer it), so "Which fund do I hold right now" / "What funds do I own?" are exempt — unless `SELECTION_INTENT_RE` also matches ("which of my funds **should I sell**?"), which is what separates a lookup from a recommendation. Both carve-outs can only ever EXEMPT a message; `ADVICE_RE` stays the final authority, so narrowing here cannot create false positives elsewhere. Tested offline by `lambda/tests/test-advice-guard.mjs` — **both halves matter**: the true positives that must still route to a callback are as load-bearing as the exemptions. **`FORBIDDEN_TOPICS` is untouched.** |
| Account cash & the balance identity | **`lambda/shared/account-math.ts`** (PURE, imported by Lambdas *and* the customer app, like `contribution-limits.ts`) — `holdingValue` / `investedValue` / `cashOf` / `accountBalance` / `recomputeAccounts` / `portfolioTotal` / `applyCashDelta`. **The invariant: `account.balance === account.cash + Σ(that account's holdings)`.** Before this there was no cash field at all: `balance` and `holdings` were authored independently, so the residual was meaningless — positive in six seeded accounts and NEGATIVE in four — while `PLACE_PURCHASE_PROMPT` offered "Cash in account" as a funding source that `execute-task` ignored entirely. A task expert handed `$4,800` by `get_accounts` and `$3,923` by `get_holdings`, with nothing relating them, told a client she had **$4,800 of cash PLUS $3,923 of holdings**. **Cash is STORED, not derived** — `clientStore.buyFund` re-marks the whole position at the live quote, so `Σholdings` moves by more than the purchase and a derived cash would absorb the mark-to-market delta (a price tick would change someone's cash). `balance` stays stored as a maintained projection; **never assign it by hand — call `recomputeAccounts`**. `cashOf` falls back to `max(0, balance − invested)` so legacy DynamoDB records written before the field still read. Semantics per task: cash-funded buy → cash down, total unchanged; bank-funded buy → cash flat, total up; **sale → proceeds become cash, total unchanged** (it used to subtract the amount *and* remove the shares, so money vanished); withdrawal/conversion → cash down, guarded. `applyCashDelta` is the single unbypassable insufficient-cash gate; the buy-funds prompt *also* checks conversationally so the client never reaches a failed submit, but the prompt is a suggestion and the handler is the rule. A cash-funded IRA buy is `type:'purchase'`, **not** `'contribution'` — moving money already inside a Roth is not new money. A **load-time invariant in `client-defaults.ts` throws** on drifting seed data, so it's a build failure rather than something an LLM explains to a client. Seed mirrors that must stay in lockstep: `client-defaults.ts`, `customer-app/src/data/personas.ts`, `agent-app/src/data/clientProfiles.ts`, `scripts/quality-loop/client-profiles.json`. Migration: reseed per env (`/reset-client-data`) — `accounts` is written wholesale so `cash` rides along. Tests: `lambda/tests/test-account-math.mjs`. |
| Client pronouns (chat + voice surfaces) | `ClientProfile.pronouns` (`lambda/shared/types.ts`) + `pronounRule(profile)` appended at autopilot-turn's 3 LLM call sites, so it reaches all 19 task experts, full-auto, and customer-bot/voice in one place. **`next-best-response` is NOT covered** — it writes to the agent, mostly in the second person, and `CHANGE_OPTIONS_SYSTEM` already tells the model to name the client rather than use a pronoun. Add `pronounRule` there too if NBR ever starts writing third-person prose about the client. The phone surfaces already did this (`prep-callback/handler.ts` — "use the client's stated pronouns... NEVER infer gender from the name"), but the chat surfaces had **no pronouns field at all** and guessed from the name. Jordan Williams is `they/them`. Seeded in all four persona mirrors; the rule renders empty when pronouns are unknown, so nothing is asserted without data. |
| ✈ scope/task SUGGESTION (agent app) | `lambda/next-best-response/handler.ts` — the deterministic guardrail at the end of the handler returns **`suggestedScope` AND `suggestedTaskId`**. It used to force `suggestedScope: 'callback'` for anything matching `TRADE_RE` (buy/sell/purchase/trade/liquidat/redeem), which dated from when trades could not be handled in chat — so "I'd like to sell some shares" suggested scheduling a callback while `place-sale` sat unoffered in the ✈ menu. It now runs the SAME matcher autopilot uses (`matchTaskByIntent` from `shared/tasks.ts`) and names the expert; `TRADE_RE` survives only as a fallback when no task matches. **Advice still routes to callback** — that genuinely needs a licensed advisor. Agent side: `ContactSlot.suggestedTaskId` (cleared everywhere `suggestedScope` is), passed as `onActivateAutopilot(scope, taskId)` from `AISupport`, and `activeAutomationLabel` renders the task name so the agent sees "Sell fund shares" rather than "Get intent". Tests: `lambda/tests/test-task-suggestion.mjs`. |
| Sell / exchange offer only HELD funds | `PLACE_SALE_PROMPT` and `EXCHANGE_FUNDS_PROMPT` take the client's real positions via `fetchHoldings()` + `formatHoldings()` (same pattern as `fetchBeneficiaries`), because `ClientProfile.holdings` is optional and the agent-app profile carries balances only. They previously injected `FUND_PICKLIST` — the whole 36-fund lineup — so a client asking to sell was offered every fund Bob's sells when she held one: *"Wait, what? You're saying that I hold all of those funds?"* A single position is now named and confirmed rather than turned into a choice. The exchange DESTINATION no longer uses the full lineup either: it assumes the client is moving into a fund they ALREADY hold, then that they know which new fund they want, and only if they say they don't know does it link **[Fund Research](/research)** — it never offers a shortlist, because a shortlist is a recommendation in disguise. **`place-sale` no longer collects a `reason`** (Withdrawal / Fund exchange / Rebalancing / Other): nothing downstream ever read it, and asking a client to justify selling their own money is intrusive — challenged on it, the expert invented a compliance rationale that was true of nothing in this system. |
| One funding source per buy; cost basis on new positions | `PLACE_PURCHASE_PROMPT` now reads holdings (`fetchHoldings`) like sale/exchange do. Two rules it lacked. (1) **Funding is all-or-nothing.** `fundingSource` is a two-option enum and `execute-task` reduces it to `/cash/i` — there is no split representation — but nothing told the model, so "invest all the idle cash and put in the rest from my bank" was accepted, recapped accurately, and rejected only at submit. The prompt now refuses the split conversationally and offers the two real choices with numbers. (2) **Cost basis election.** When a purchase OPENS a position in a **taxable** account, the expert asks for Average cost / Specific-lot identification / FIFO (the three in `kb.ts` q-cb-002), and `execute-task` writes it onto the newly created holding — collected-then-discarded is the `fundingSource` bug's exact shape. It does NOT ask when adding to an existing position (a method is already on file) or in an IRA, where no sale is taxed and basis is never reported (`kb.ts` q-cb-003). The registry field is `required:false` + `requiresAccountTypes:['Taxable Account']`; the "new position" half can't be expressed there, so the prompt gates it. |
| Exchange is one-to-one until the client says otherwise | `EXCHANGE_FUNDS_PROMPT` phrases every question as a single fund-to-fund move and never raises splitting. If the CLIENT asks for many-to-one / one-to-many / many-to-many, that's legitimate: the expert works it through, but each submitted exchange moves ONE source to ONE destination, so it recaps the set and handles the first pairing. It also states the **amount ceiling** — `execute-task` rejects `amount > fromPosition.value` and the prompt previously never said so, the same "promise what execution rejects" defect as the split funding above. |
| Task experts know site URLs, and never recommend a fund | The `FORBIDDEN_TOPICS_NO_TRADES` block (shared by the five fund-facing experts: buy, sell, exchange, setup/update auto-invest) now carries the portal paths they may link — `/research`, `/research/fund/TICKER`, `/help/cost-basis`, `/help/fees`, `/help/place-trade` — because task experts previously had no notion that pages existed; only the customer-bot prompt did, via `SELF_SERVICE_PAGES` (which was itself missing `/research`, now added). Same block draws the legal line: experts ANSWER factual fund questions (objective, expense ratio, holdings, performance — via `get_funds`) but must never say which fund is better, safer, a fit, popular, or what they'd do. Comparing on published facts is service; concluding what to buy is advice. |
| Live account figures in every expert prompt | `autopilot-turn` had a `fetchHoldings` and **no `fetchAccounts`**: it used the request body's `clientProfile` verbatim, and the agent app posts a hardcoded literal (`agent-app/src/data/clientProfiles.ts`) that nothing refreshed. One prompt therefore carried positions read live from DynamoDB beside balances frozen at seed time — a client was told *"$67,890 in total ... $67,890 already invested ... and no cash available"* of an account holding **$68,890 with $6,385 cash**. Now `fetchAccounts` + **one override before every scope branch**, so ~20 `summarizeAccounts` headers and all 19 experts get live figures from a single line (only `accounts`/`totalBalance` are replaced — `intents`/`pronouns` exist only in the posted profile). Cash facts are injected by **`describeCashAvailability`** (`lambda/shared/account-math.ts`, reusing `cashOf`/`investedValue`), the account analogue of `describeCallbackAvailability`: the server states the figure, the model never derives it. Its **zero-cash branch forbids mentioning balances at all** — reciting fund balances answers nothing when the question is how to pay. A failed read degrades to **silence, not stale data**: every expert is told to state no figure, including those on its own header. `summarizeAccounts` can no longer emit a nounless dollar amount. The agent app fetches live accounts too (`agent-app/src/data/liveProfile.ts` → `client-data` `get-all`), so agent and model stop quoting different numbers. **Prompt prose is not a fix for this class of bug:** three successive attempts to add rules grew `PLACE_PURCHASE_PROMPT` 79→154 lines and produced three *different* wrong cash figures (a holding's value, a number from the prompt's own worked example, the identity inverted) — the model recites the vocabulary of rules written about numbers. It is now 122 lines and every line is a fact or a capability rule. |
| ✈ suggestion names the expert, not the scope | Accepting a contact hardcoded `suggestedScope:'get-intent'` with **no task** (`ChatColumn.tsx`), and `runNbrRefresh` — which attaches the task — is gated on `lastCustomerMessageAt`, set only by the live chat socket. The buy intent is stated **before** escalation, so it never fired and the agent saw a bare "Get intent" on the least ambiguous intent in the system. The deterministic guardrail now lives in **`lambda/shared/suggest-scope.ts`** (`resolveSuggestion`), and `next-best-response` gained a **`mode:'scope-only'`** branch that returns it *before any Bedrock call* (~0.14s, free), called the instant a contact is accepted. `agentStore.setSuggestion` writes scope+task as one fact and **`patchSlot` is narrowed to forbid setting either alone**, which turned this bug class into a compile error and surfaced three more leaks (the autopilot-turn response, the idle timer, a failed NBR refresh). `taskLabel('place-purchase')` → **"Buy funds"**. |
| Client-facing reply hygiene | `lambda/shared/reply-hygiene.ts` — `stripInternalStatus(response, exitMessage)`, applied at both task-expert parse sites in `autopilot-turn`. A task-expert turn returns two strings for **two different audiences**: `response` goes VERBATIM to the client, `exitMessage` is read only by the human agent. `EXIT_MESSAGE_INSTRUCTION` shows the model an example exitMessage, and an expert appended it to the response — so a client was told mid-purchase *"Great, I'll proceed with that. All fields collected — proposed action is ready for review."* The instruction now spells out the two audiences and forbids mixing them; this function is the deterministic backstop. It is **sentence-granular** (the surrounding reply is nearly always good — only the bookkeeping clause goes) and deliberately narrow, so an ordinary "send you a statement to review" survives. If stripping empties the reply, a neutral line is sent rather than a blank message. Tests: `lambda/tests/test-strip-internal-status.mjs`. |
| Money parsing in task execution | `lambda/shared/money.ts` — `parseMoney` / `isValidAmount` / `isFullAmount` / `resolveAmount` / `numberOr`. Task-expert prompts tell the LLM to write amounts as `"$5,000"`, and `parseFloat("$4,800")` is `NaN` — while `NaN <= 0` is **false**, so every `amount <= 0` guard in `execute-task` passed NaN into the holdings/balance math, DynamoDB rejected it, and the agent got an opaque "Submission failed". Four experts (place-sale, exchange-funds, request-withdrawal, roth-conversion) also *invite* "full balance"/"all shares", which hit `parseFloat` as a phrase — so the **documented answer** failed too; `resolveAmount` resolves those against a ceiling the caller supplies. Use `isValidAmount`, never `<= 0`. Validation failures return **HTTP 200 with `success:false`** and a real message, because `ProposedActionCard` renders `result.message` for that shape while a 4xx/5xx throws into a bare `catch` and degrades to the generic string. Tests: `lambda/tests/test-money.mjs`. |
| Demo clock (seeded "now") | `lambda/shared/transaction-status.ts` → `DEMO_TODAY`, a **hand-bumped** constant (not `new Date()`, so the seeded PRNG stays deterministic and a reseed is idempotent). Everything dated relative to "now" derives from it: the generated ledger span (`transaction-history.ts`), `autoInvest[].nextDate`, the RMD block's deadline + distribution years, and `genBalanceHistory`'s window (`client-defaults.ts`). **Refresh ritual** (run periodically, and before any demo showing year-to-date figures — the contributions card sums real calendar years, so a stale clock reads as "$0 contributed this year"): bump `DEMO_TODAY` to a weekday → deploy → `GET /reset-client-data?key=bobs-reset-2025`. Step 3 wipes and regenerates every persona's ledger. |
...
```
{
  ...
  totalBalance,
  accounts: [{type, balance, cash, id, change}],   // balance = cash + Σ holdings (account-math.ts)
...
Fund Market Table (`bobs-fund-market`): REAL market data mirrored nightly from each fund's
Vanguard proxy by `fund-data-refresh`. PK `ticker` / SK `section`: `latest` (the page-ready
`FundMarketData` payload — quote, trailing/annual total returns, holdings, sectors, characteristics,
distributions, downsampled chart series — plus `historyHashes` bookkeeping), `history#<year>`
(raw daily closes, content-hashed so unchanged years are never rewritten), `dividends` (full
distribution event history), and reserved partitions `_summary`/`_status` (underscore can't collide
with a ticker). Fully re-derivable from Yahoo on any run → no PITR, DESTROY. The shared type
contract is `customer-app/src/data/fundMarket.ts` (bridged via `lambda/shared/fund-market-types.ts`,
fund-catalog pattern). Static catalog data stays in `bobs-funds`; this table is the live/history side.
...
**In flight (`feature/ira-contributions-summary`, PR open): IRA contributions summary.**
Every IRA account page gains a **Contributions** card — year-to-date contributed, the
age-adjusted annual limit, remaining room, a progress bar, a per-account breakdown, and a
**Contribute →** button — and the same numbers are available to every conversational AI
surface via one new tool. The headline figure is deliberately portfolio-wide, because the
IRS applies the limit **across all of a person's IRAs combined**, not per account. See the
three new Key Files rows: *IRA contributions summary*, *Financial-advice guard*, *Demo clock*.

- **Computed once, server-side** (`lambda/shared/contributions.ts`) so the card, the chatbot,
  the voice assistant, the agent's suggested replies, and the phone-cockpit dossier can never
  disagree about a number that carries a 6% excise tax for being wrong.
- **SEP-IRA is a separate bucket** with its own limit and **no computed remaining** — the real
  SEP limit is 25% of compensation, which we do not hold, so the card says so instead of
  inventing a figure.
- **Three stated assumptions** (`CONTRIBUTION_ASSUMPTIONS`) render verbatim in BOTH the card
  and the AI tool output: contributions made at other firms are not included (they count
  toward the same limit), the earned-income rule is not checked, and Roth MAGI phase-outs are
  not applied.
- **Prerequisite shipped with it: the demo clock moved to 2026-08-20.** The seeded ledger was
  frozen at 2025-04-15, so a real-calendar "year to date" summed zero rows. `DEMO_TODAY` is now
  documented as a hand-bumped constant with a refresh ritual, and everything dated relative to
  "now" derives from it. **This means the prod reseed is part of shipping** —
  `GET /reset-client-data?key=bobs-reset-2025` — and it wipes/regenerates every persona's ledger.
- **Note on the prior-year panel:** it correctly renders only while the prior year's deadline
  is open (Jan 1 – Apr 15). Today it is hidden, which is right but means half the feature is
  invisible in a demo until January. The `asOf` override on `get-contributions` is how it is
  exercised; `test-contributions.mjs` uses it.
- **⚠ The 2026 IRS figures need a human check** — $7,500 base / $1,100 catch-up / $72,000 DC cap,
  and the Roth phase-out ranges, all in `CONTRIBUTION_LIMITS`. They drive customer-facing dollar
  amounts. One table, one edit.
- Verified on dev: 42/42 offline math, 70/70 API across all four personas, a full Playwright UI
  pass, a live buy round-trip (+$250 in → contributed +$250, remaining −$250), the bot/full-auto/
  get-intent answers quoting the exact figure with the caveat, and 11/11 on the existing
  `test-force-task` routing suite (no regression from the advice-guard refactor).
