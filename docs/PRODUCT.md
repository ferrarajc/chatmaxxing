...
- **Autopilot controls:** While a column is running on autopilot, the message composer is replaced by a large **Pause Autopilot** button. Pausing *freezes* the countdown to the next auto-send without exiting — the paused state offers **Exit** (end autopilot for that column) and **Resume ▶** (continue from exactly where it left off). Clicking elsewhere in the column, or dragging the AI-panel resize handle to read a longer draft, no longer exits autopilot — only these controls do. The pending auto-send shows a live countdown that flashes red in its final 10 seconds so the agent can intervene in time. When paused, the pending-send box itself becomes a compact control bar: the countdown slides to the left, **‹ ›** chevrons flip between alternative drafts, a **Send now** button dispatches the shown reply immediately (skipping the rest of the countdown), and the same **🪄 Magic** and **Change to** tools from the Suggested reply are available to reshape the message in place — so the agent can perfect the reply and either send it at once or let the countdown resume on Resume. Autopilot is also resilient to hiccups: a transient AI error no longer drops the agent out of a task mid-collection (it retries once and, if it must stop, says so with a message that can be resumed), and a client reply arriving while the agent is mid-edit can never cause a duplicate send.
